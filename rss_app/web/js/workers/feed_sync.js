...
//                + currentFeed.pageurl + ', new: ' + feedNewData.pageurl);
    ...
    if (feedChanged) {
        rss.db.updateFeed(currentFeed);
    }
    
    rss.db.getEntries(rss.sync.currentFeed.id).then(function(posts) {
        ...
        console.error('Called error on getEntries.');
        throw e;
    });
};
/**
...
rss.sync._syncContent = function() {
    ...
    inserts = rss.sync.extractImages(inserts);
...
rss.sync.extractImages = function(inserts) {
    for (var i = 0, len = inserts.length; i < len; i++) {
        inserts[i].imageUrl = rss.utils.extractImageUrl(inserts[i].content);
    }
    return inserts;
};
...
rss.sync.update = function(inserts, updates) {
    if (inserts.length > 0 || updates.length > 0) {
        rss.db.insertPosts(inserts, updates)
        .then(function() {
            rss.db.close();
